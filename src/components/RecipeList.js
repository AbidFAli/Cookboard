...
function RecipeList(props){
  ...
  if(props.recipes == null || props.recipes.length === 0){
      return (<Typography variant = "subtitle1" gutterBottom>{props.messageNoContent}</Typography>)
  }
  else if (props.recipes != null) {
      content = props.recipes.map((recipe, index) => {
          return (
              
                  <ListItem key={index} button onClick={() => history.push(`${PATH_RECIPES}/${recipe.id}`)}>
                      <ListItemIcon>
