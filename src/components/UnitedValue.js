...
const UnitedValue = (props) => {
  ...
  let inputPropsValue =
    props.testIdValue != null ? { "data-testid": props.testIdValue } : null;
  let inputPropsUnit =
    props.testIdUnit != null ? { "data-testid": props.testIdUnit } : null;

  const setErrorMessages = (errorInfo) => {
    setUnitErrorMessage(
      errorInfo.errorMessageUnit !== undefined
        ? errorInfo.errorMessageUnit
        : null
    );
    setValueErrorMessage(
      errorInfo.errorMessageValue !== undefined
        ? errorInfo.errorMessageValue
        : null
    );
  };
