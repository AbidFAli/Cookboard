...
describe("Tests for RecipePage", () => {
  ...
  test("blank recipes start in editing mode", async () => {
    await testHelper.setupAndRenderRecipe({ user: userFixture() });
  ...
  test("recipes cannot be edited or deleted without a user", async () => {
    await testHelper.setupAndRenderRecipe({ recipe: recipeFixture() });
    expect(screen.queryByTestId(ID_EDIT_BUTTON)).toBeNull();
  ...
  describe("tests for recipe creation", () => {
    let history;
    beforeEach(async () => {
      ({ history } = await testHelper.setupAndRenderRecipe({
        user: userFixture(),
      }));
    });
    ...
    test("recipes can be created", async () => {
      ...
      recipeService.create.mockResolvedValueOnce({
        name: recipeName,
        id: "1234",
      });
      testHelper.clickSave();
      expect(recipeService.create.mock.calls[0][0]).toMatchObject({
        name: recipeName,
      });
    ...
    test("A message will be displayed to the user if a TokenExpiredError occurs", async () => {
      ...
      testHelper.clickSave();
      expect(
        await screen.findByText(MESSAGE_TOKEN_EXPIRED)
      ).toBeInTheDocument();
    });
  });
  ...
  describe("tests for ratings", () => {
    ...
    test("if the rating update fails, show a failure notification", async () => {
      screen.queryBy;
    });

    test("if the rating update succeeds, show a success notification", async () => {});
  });
});
