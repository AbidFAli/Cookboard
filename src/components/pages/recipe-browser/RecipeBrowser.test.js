...
import { errorMessages as optionsErrors, ids as optionsIds } from './RecipeBrowserOptions';
...
async function performSearch(results, totalResults){
  ...
  await waitFor(() => expect(recipeService.search.mock.calls.length).toBeGreaterThan(initialCalls))
}
...
async function viewNextRecipes(recipes, currentPos, resultSize = 5){
      ...
      await waitFor(() => expect(resultsInfo).toHaveTextContent(`Showing results from ${currentPos + 1} to ${currentPos + nextRecipes.length} of ${recipes.length} results`))
...
async function viewPreviousRecipes(recipes, currentPos, resultSize = 5){
  ...
  await waitFor(() => expect(resultsInfo).toHaveTextContent(`Showing results from ${start + 1} to ${start + nextRecipes.length} of ${recipes.length} results`))
  return start
}
...
describe('tests for RecipeBrowser', () => {
  ...
  describe('tests for BrowserSearchOptions', () => {
    test('if ratingMax < ratingMin an error will be displayed', async () => {
      setRatingMax(2)
      setRatingMin(4)
      expect(await screen.findByText(optionsErrors.ERROR_MESSAGE_MAX_LESS_MIN)).toBeInTheDocument()
    })

    test('if ratingMax is out of range [0,5] an error will be displayed', async () => {
      setRatingMax(7)
      expect(await screen.findByText(optionsErrors.ERROR_OUT_OF_RANGE)).toBeInTheDocument()
    })
  ...
  test('searching displays the first set of results and a count of the total number of results', async () => {
    ...
    await performSearch(response, totalResults)
  ...
  describe('tests for pagination', () => {
    ...
    beforeEach(async () => {
      ...
      await performSearch(testRecipes.slice(0,5), testRecipes.length)
      currentPos = 5;
    })
    ...
    test('the next button is disabled on the last page', async () => {
      ...
      for(let i = 1; i < numBatches; i++ ){
        let nextRecipes = testRecipes.slice(i*batchSize,(i+1) * batchSize)
        recipeService.search.mockResolvedValueOnce(nextRecipes)
        userEvent.click(nextButton)
        await waitFor(() => expect(resultsInfo).toHaveTextContent(`${i*batchSize + 1} to ${i*batchSize + nextRecipes.length} of ${testRecipes.length}`))
      }
    ...
    describe('on the second page', () => {
      ...
      test('clicking the back button shows the previous set of results', async () => {
        ...
        for(let i = 0; i < 5; i++){
          expect(await screen.findByText(testRecipes[i].name)).toBeInTheDocument()
        }
      })
    })
