...
function RecipeBrowser(props){
  const [searchOptions, setSearchOptions] = useState({name: '', ingredient: '', ratingMin: 0, ratingMax: 5})
  ...
  const performNewSearch = async () => {
    setSearching(true)
    let resultsTotal = await recipeService.getNumberSearchResults(searchOptions)
  ...
  const handleNext = async () => {
    let results = await recipeService.search(searchOptions, (batchNumber + 1)* resultSize, resultSize)
    setRecipes(results)
  ...
  const searchButton = (
    <Button data-testid = {ID_SEARCH_BUTTON} onClick = {performNewSearch}>{TEXT_SEARCH_BUTTON}</Button>
  )
  ...
  const resultsInfo = (
    <Grid item>
      <Typography variant = "subtitle1" data-testid = {ID_RESULTS_INFO}>
        Showing results from {currentPos+1} to {currentPos + recipes.length} of {resultsTotal} results
      </Typography>
    </Grid>
  )
      ...
      <MobileStepper
        ...
        nextButton = {
          <Button 
            ...
            data-testid = {ID_NEXT_BUTTON}>Next</Button>
        }
        backButton = {
          <Button 
            size = "small" 
            ...
            data-testid = {ID_BACK_BUTTON}>Back</Button>
        }
      />
    </Grid>
  )
    ...
    <Grid container direction= "column">
      ...
      <Grid item container direction = "row-reverse">
        <Grid item container xs = {12} md = {4} direction = "column">
          <RecipeBrowserOptions searchOptions = {searchOptions} updateSearchOptions = {setSearchOptions} />
        </Grid>
        <Grid item container xs ={12} md = {8} direction = "column">
          <Paper>
            <RecipeList recipes={recipes} messageNoContent = {INITIAL_LIST_MESSAGE} />
          </Paper>
        </Grid>
      </Grid>
      {searching && stepper}
